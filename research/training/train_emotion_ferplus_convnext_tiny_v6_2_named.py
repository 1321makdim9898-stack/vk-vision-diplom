# train_emotion_ferplus_convnext_tiny_v6_1.py
import os
import json
import time
import math
...
def _save_matrix_csv(path: str, matrix: np.ndarray, labels: List[str], float_fmt: Optional[str] = None):
    with open(path, "w", encoding="utf-8") as f:
        ...
        for i, row in enumerate(matrix):
            if float_fmt is None:
                ...
                vals = [format(float(v), float_fmt) for v in row.tolist()]
            f.write(labels[i] + "," + ",".join(vals) + "\n")


def _plot_matrix(path: str, matrix: np.ndarray, labels: List[str], title: str, value_fmt: str = "int"):
    ...
    thresh = max_val * 0.6 if max_val > 0 else 1.0
    for i in range(matrix.shape[0]):
        for j in range(matrix.shape[1]):
            v = matrix[i, j]
            if value_fmt == "pct":
                text_v = f"{float(v) * 100:.1f}%"
            elif value_fmt == "float":
                text_v = f"{float(v):.4f}"
            else:
                text_v = str(int(v))
            plt.text(j, i, text_v, ha="center", va="center", color="white" if v > thresh else "black")
...
def save_confusion(out_dir: str, prefix: str, y_true: np.ndarray, y_pred: np.ndarray, labels: List[str]):
    ...
    _save_matrix_csv(os.path.join(out_dir, f"{prefix}_confusion_matrix.csv"), cm, labels)
    _plot_matrix(os.path.join(out_dir, f"{prefix}_confusion_matrix.png"), cm, labels, f"{prefix} confusion matrix", value_fmt="int")

    row_sums = cm.sum(axis=1, keepdims=True)
    cm_norm = np.divide(cm.astype(np.float64), np.maximum(row_sums, 1.0), where=np.maximum(row_sums, 1.0) > 0)
    _save_matrix_csv(os.path.join(out_dir, f"{prefix}_confusion_matrix_norm.csv"), cm_norm, labels, float_fmt=".6f")
    _plot_matrix(os.path.join(out_dir, f"{prefix}_confusion_matrix_norm.png"), cm_norm, labels, f"{prefix} normalized confusion matrix", value_fmt="pct")


def save_per_class_metrics(out_dir: str, prefix: str, y_true: np.ndarray, y_pred: np.ndarray, labels: List[str]):
    if not SKLEARN_OK:
        return
    ...
    csv_path = os.path.join(out_dir, f"per_class_metrics_{prefix}.csv")
    with open(csv_path, "w", encoding="utf-8") as f:
        f.write("class,precision,recall,f1,support\n")
        for label, p, r, f1v, sup in zip(labels, precision, recall, f1, support):
            f.write(f"{label},{p:.6f},{r:.6f},{f1v:.6f},{int(sup)}\n")
    ...
    plt.ylim(0.0, 1.0)
    plt.ylabel("score")
    plt.title(f"Per-class metrics ({prefix})")
    plt.grid(True, axis="y", alpha=0.25)
    plt.legend()
    plt.tight_layout()
    fig.savefig(os.path.join(out_dir, f"per_class_metrics_{prefix}.png"), dpi=160, bbox_inches="tight")
    plt.close(fig)


def append_experiment_summary(run_dir: str, cfg: Dict, best_summary: Dict, history: List[Dict]):
    if history:
        best_epoch_row = max(history, key=lambda h: (h.get("val_acc", -1.0), -(h.get("val_loss", 1e9) or 1e9)))
        final_row = history[-1]
    else:
        best_epoch_row = {}
        final_row = {}
    ...
    row = {
        ...
        "lr": cfg.get("lr"),
        "lr_head_mult": cfg.get("lr_head_mult"),
        "dropout": cfg.get("dropout"),
        "scheduler": cfg.get("scheduler"),
        "balanced_sampler": cfg.get("balanced_sampler"),
        "use_class_weights": cfg.get("use_class_weights"),
        "contempt_to": cfg.get("contempt_to"),
        "mixup_alpha": cfg.get("mixup_alpha"),
        "cutmix_alpha": cfg.get("cutmix_alpha"),
        "mix_prob_start": cfg.get("mix_prob"),
        "mix_prob_end": cfg.get("mix_prob_end"),
        "best_epoch": best_epoch_row.get("epoch"),
        "best_val_acc": best_summary.get("best_val_acc"),
        "best_test_acc": best_summary.get("best_test_acc"),
        "best_val_f1_macro": best_epoch_row.get("val_f1_macro"),
        "best_test_f1_macro": best_epoch_row.get("test_f1_macro"),
    ...
    out_csv = os.path.join(run_dir, "experiment_summary.csv")
...
def write_report_md(run_dir: str, cfg: Dict, best: Dict, class_counts: Dict[str, Dict[str, int]], notes: List[str]):
    lines = []
    lines.append("# Emotion (FERPlus→7) v6 ConvNeXt — отчёт обучения\n")
...
def build_model_convnext_tiny(num_classes: int, dropout: float):
...
class TrainConfig:
    ...
    # mapping
    contempt_to: str = "drop"  # disgust|neutral|drop
...
def main():
    ...
    ap.add_argument("--run_name", default=f"emotion_ferplus_convnext_tiny_v6_2_{now_tag()}")
    ...
    ap.add_argument("--contempt_to", default="drop", choices=["disgust", "neutral", "drop"])
    ...
    ap.add_argument("--mix_prob", type=float, default=0.3)
    ap.add_argument("--mix_prob_end", type=float, default=0.10)
    ...
    ap.add_argument("--force_class_weights", action="store_true", help="Enable class weights even if sampler is disabled")
    ...
    cfg = TrainConfig(
        ...
        use_class_weights=bool(args.force_class_weights),
    ...
    # model
    model = build_model_convnext_tiny(num_classes=len(APP_LABELS), dropout=cfg.dropout).to(device)

    # avoid double rebalancing: sampler + class weights at the same time
    if cfg.balanced_sampler and cfg.use_class_weights:
        print("[INFO] balanced_sampler=True -> disable class weights to avoid double rebalancing")
    ...
    best_by_val_run = os.path.join(run_dir, "emotion_convnext_tiny_ferplus_v6_2_best_val.pth")
    best_by_test_run = os.path.join(run_dir, "emotion_convnext_tiny_ferplus_v6_2_best_test.pth")

    best_by_val_models = os.path.join(models_dir, "emotion_convnext_tiny_ferplus_v6_2_best_val.pth")
    best_by_test_models = os.path.join(models_dir, "emotion_convnext_tiny_ferplus_v6_2_best_test.pth")
    ...
    if cfg.eval_only:
        ...
        if SKLEARN_OK:
            ...
            save_per_class_metrics(out_dir, "val", val_res["y_true"], val_res["y_pred"], APP_LABELS)
            save_confusion(out_dir, "test", test_res["y_true"], test_res["y_pred"], APP_LABELS)
            save_per_class_metrics(out_dir, "test", test_res["y_true"], test_res["y_pred"], APP_LABELS)
    ...
    notes = [
        ...
        "Model: ConvNeXt-Tiny ImageNet-pretrained + dropout",
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        if improved_val:
            ...
            if SKLEARN_OK and val_res["y_true"].size:
                ...
                save_per_class_metrics(run_dir, "val", val_res["y_true"], val_res["y_pred"], APP_LABELS)
                vr = classification_report(val_res["y_true"], val_res["y_pred"], target_names=APP_LABELS, digits=4, zero_division=0)
        ...
        if improved_test:
            ...
            if SKLEARN_OK and test_res["y_true"].size:
                ...
                save_per_class_metrics(run_dir, "test", test_res["y_true"], test_res["y_pred"], APP_LABELS)
                tr = classification_report(test_res["y_true"], test_res["y_pred"], target_names=APP_LABELS, digits=4, zero_division=0)
    ...
    append_experiment_summary(run_dir, asdict(cfg) | {"backbone": "convnext_tiny"}, best_summary, history)
    write_report_md(run_dir, asdict(cfg) | {"backbone": "convnext_tiny"}, best_summary, class_counts, notes)
