# train_emotion_ferplus_resnet18_v2.py
import os
import json
import time
import math
...
def write_report_md(run_dir: str, cfg: Dict, best: Dict, class_counts: Dict[str, Dict[str, int]], notes: List[str]):
    ...
    lines.append("- `confusion_matrix.png`, `confusion_matrix.csv` — confusion matrix (val)")
    lines.append("- `classification_report.txt` — classification report (val)")
...
def compute_class_weights(items: List[Tuple[str, int]]) -> torch.Tensor:
    ...
    cnt = np.array([1, 1, 1, 1, 1, 1, 1], dtype=np.float32)
    for _, y in items:
        cnt[int(y)] += 1
    inv = 1.0 / cnt
    w = inv / inv.mean()
...
class FERPlusMappedDataset(Dataset):
    ...
    Читает структуру:
      ...
      root/test/<class>/*
    Где классы FERPlus: angry, contempt, disgust, fear, happy, neutral, sad, suprise/surprise.
...
def build_transforms(img_size: int):
    # FERPlus в твоих скринах — уже face crops, но всё равно grayscale.
    # ResNet ждёт 3 канала — делаем Grayscale(num_output_channels=3).
...
def main():
    ...
    ap.add_argument("--run_name", default=f"emotion_ferplus_resnet18_v2_{now_tag()}")
    ...
    # model
    model = build_model(num_classes=len(APP_LABELS), dropout=cfg.dropout).to(device)
    ...
    # куда сохранять веса (не трогаем старые!)
    best_ckpt_path_run = os.path.join(run_dir, "emotion_resnet18_ferplus_v2_best.pth")
    models_dir = os.path.abspath(os.path.join(os.getcwd(), "models"))
    ensure_dir(models_dir)
    best_ckpt_path_models = os.path.join(models_dir, "emotion_resnet18_ferplus_v2_best.pth")
    ...
    notes = [
        ...
        "Модель: ResNet18 ImageNet-pretrained + dropout",
