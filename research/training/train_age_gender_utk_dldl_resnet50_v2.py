# train_age_gender_utk_dldl_resnet50_v2.py
...
# ✅ ВАЖНО: выключаем GUI-backend matplotlib (исправляет Tkinter/Tcl_AsyncDelete на Windows)
import matplotlib
...
def parse_utk_filename(fn: str) -> Optional[Tuple[int, int]]:
    """
    UTKFace filename often: age_gender_race_date.jpg
    ...
    if not m:
        return None
    age = int(m.group(1))
    gender = int(m.group(2))  # 0 female, 1 male (in UTKFace)
...
def make_gaussian_label(age: int, sigma: float = 2.0) -> torch.Tensor:
    """
    DLDL target distribution over ages 0..120.
    Gaussian centered at true age.
    """
    xs = torch.arange(0, N_AGE, dtype=torch.float32)
    mu = float(age)
    s2 = float(sigma) ** 2
    dist = torch.exp(-0.5 * (xs - mu) ** 2 / s2)
...
class UTKFaceCropsDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        # ✅ возвращаем уже тензоры, чтобы дальше не делать torch.tensor(...) и не ловить warning
        age_t = torch.tensor(age, dtype=torch.long)
        gender_t = torch.tensor(gender, dtype=torch.float32)
...
class AgeGenderDLDLResNet50(nn.Module):
    def __init__(self, pretrained: bool = True, dropout: float = 0.2):
        ...
        self.age_head = nn.Linear(in_f, N_AGE)     # logits over age classes
        self.gender_head = nn.Linear(in_f, 1)      # logit

    def forward(self, x):
        f = self.backbone(x)
        f = self.drop(f)
        age_logits = self.age_head(f)
...
# -----------------------------
@torch.no_grad()
def expected_age_from_logits(age_logits: torch.Tensor) -> torch.Tensor:
    probs = F.softmax(age_logits, dim=1)
    xs = torch.arange(0, N_AGE, device=probs.device, dtype=probs.dtype).unsqueeze(0)
    exp_age = (probs * xs).sum(dim=1)
    return exp_age
...
def train_one_epoch(model, loader, optimizer, scaler, device, cfg: TrainConfig):
    ...
    for x, age, gender, _ in tqdm(loader, desc="train", leave=False):
        ...
        # ✅ age/gender уже тензоры от датасета, просто переносим на устройство
        age = age.to(device, non_blocking=True)
        gender = gender.to(device, non_blocking=True)

        tgt = torch.stack([make_gaussian_label(int(a.item()), cfg.sigma) for a in age], dim=0).to(device)

        optimizer.zero_grad(set_to_none=True)

        # ✅ новый AMP API (убирает FutureWarning)
        with torch.amp.autocast("cuda", enabled=cfg.amp and device.type == "cuda"):
            age_logits, gender_logit = model(x)

            logp = F.log_softmax(age_logits, dim=1)
            loss_age = F.kl_div(logp, tgt, reduction="batchmean")
        ...
        with torch.no_grad():
            pred_age = expected_age_from_logits(age_logits)
            abs_err = torch.abs(pred_age - age.float()).sum().item()

            total_abs_err += abs_err

            pred_bins = torch.tensor([bin_idx_from_age(a.item()) for a in pred_age], device=device)
            true_bins = torch.tensor([bin_idx_from_age(a.item()) for a in age.float()], device=device)
...
def eval_one_epoch(model, loader, device, cfg: TrainConfig):
    ...
    for x, age, gender, _ in tqdm(loader, desc="val", leave=False):
        ...
        # ✅ eval без AMP (как и было), но без deprecated autocast
        ...
        pred_age = expected_age_from_logits(age_logits)
        all_true_age.append(age.float().cpu().numpy())
...
def main():
    ...
    ap.add_argument("--data_root", required=True, help=r'например: C:\vk-vision-demo\data\UTKFace\utkface_mediapipe_cropped')
    ap.add_argument("--out_root", default="runs_age_gender", help="куда складывать результаты обучения")
    ap.add_argument("--run_name", default=f"dldl_resnet50_v2_{now_tag()}")
    ...
    model = AgeGenderDLDLResNet50(pretrained=cfg.pretrained, dropout=cfg.dropout).to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=cfg.lr, weight_decay=cfg.weight_decay)

    # ✅ новый GradScaler API (убирает FutureWarning)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        if row["val_mae"] < best_mae:
            ...
            best_path = os.path.join(run_dir, "age_gender_dldl_resnet50_v2_best.pth")
            torch.save(
                {
        ...
        if epoch % 10 == 0 or epoch == cfg.epochs:
            ckpt_path = os.path.join(run_dir, f"age_gender_dldl_resnet50_v2_e{epoch:02d}.pth")
